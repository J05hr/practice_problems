...
class Solution:
    ...
    def get_cheapest_cost(self, rootNode):
        ...
        self.cheap(rootNode, current_sum, current_min)
        return current_min

    def cheap(self, rootNode, current_sum, current_min):
        current_sum += rootNode.cost

        if current_sum < current_min and len(rootNode.children) == 0:
            current_min = current_sum

        for child in rootNode.children:
            self.cheap(child, current_sum, current_min)
...
if __name__ == '__main__':
    tester = Solution()
    tree = None
    ans = tester.get_cheapest_cost(tree)
    print(ans)
